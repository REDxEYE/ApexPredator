...
static inline void Archive_has_file(const Archive* archive, const String* path) {
    if(archive && archive->has_file) {
        archive->has_file(archive, path);
    }
}

static inline void Archive_has_file_by_hash(const Archive* archive, uint32 hash) {
    if(archive && archive->has_file_by_hash) {
        archive->has_file_by_hash(archive, hash);
    }
}

static inline bool Archive_get_file(Archive* archive, const String* path, MemoryBuffer* out) {
    if(archive && archive->get_file) {
...
static inline bool Archive_get_file_by_hash(Archive* archive, uint32 hash, MemoryBuffer* out) {
    if(archive && archive->get_file_by_hash) {
