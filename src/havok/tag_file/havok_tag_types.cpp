...
void HKTagType_free(HKTagType *type) {
    ...
    String_free(&type->stable_name);
    DA_free_with_inner(&type->members, {HKTagTypeMember_free((HKTagTypeMember*)it);});
    DA_free(&type->interfaces);
    DA_free_with_inner(&type->template_args, { HKTagTemplateArgument_free((HKTagTemplateArgument*)it);});
}
