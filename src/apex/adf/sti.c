...
void STI_TypeLibrary_init(STI_TypeLibrary *lib) {
    ...
    //Deferred = 0xDEFE88ED
    type = DM_insert(&lib->types, STI_TYPE_HASH_DEFERRED);
...
STI_Type *STI_TypeLibrary_register_adf_type(STI_TypeLibrary *lib, const ADF *adf, const ADFType *adf_type) {
    ...
    STI_Type *type = DM_get(&lib->types, adf_type_hash);
    if (type != NULL) {
        return type;
    }
    ...
    if (adf_type->def.type == ADF_Bitfield) {
        ...
        const STI_Type *inner_type = DM_get(&lib->types, adf_type->def.element_type_hash);
        if (inner_type == NULL) {
    ...
    type = DM_insert(&lib->types, adf_type_hash);

    const uint32 *other_adf_hash = NULL;
    if ((other_adf_hash = DM_get(&lib->already_seen_name_hashes, name_hash)) != NULL) {
    ...
    switch ((STI_DataType) adf_type->def.type) {
        case STI_Structure: {
            ...
            for (int i = 0; i < adf_type->type_data.struct_data.members.count; ++i) {
                ...
                STI_StructMember *member = DA_append_get(&type->data.struct_data.members);
        ...
        case STI_Enumeration: {
            ...
            for (int i = 0; i < adf_type->type_data.enum_data.members.count; ++i) {
                ...
                STI_EnumMember *member = DA_append_get(&type->data.enum_data.members);
                String_copy_from(&member->name, &adf->strings.items[adf_member->name_id]);
