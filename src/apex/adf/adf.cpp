...
bool read_typedef(ADF *adf, Buffer *buffer) {
    ADFType *type = (ADFType *) DA_append_get(&adf->types);
    ...
    switch (typedef_->type) {
        case ADF_Structure: {
            ...
            for (int j = 0; j < member_count; ++j) {
                ADFEnumMemberInfo *member = (ADFEnumMemberInfo *) DA_append_get(&type->type_data.struct_data.members);
        ...
        case ADF_Enumeration: {
            ...
            for (int j = 0; j < member_count; ++j) {
                ADFEnumMemberInfo *member = (ADFEnumMemberInfo *) DA_append_get(&type->type_data.enum_data.members);
...
bool ADF_from_buffer(ADF *adf, Buffer *buffer) {
    ...
    for (int i = 0; i < header->nametable_count; ++i) {
        buffer->read_cstring(buffer, (String *) DA_append_get(&adf->strings));
    }
    ...
    for (int i = 0; i < header->instance_count; ++i) {
        ADFInstance *instance = (ADFInstance *) DA_append_get(&adf->instances);
...
void ADF_free(ADF *adf) {
    ...
    DA_free_with_inner(&adf->strings, {String_free((String*)it);});
    DA_FORI(adf->types, i) {
        ADFType_free((ADFType *) DA_at(&adf->types, i));
    }
    DA_free(&adf->types);
...
ADF *ADF_load_builtin_adf(const uint8 *data, int64 size) {
    MemoryBuffer emb = {};
    ADF *adf = (ADF *) mp_calloc(sizeof(ADF), 1);
...
ADFInstance *ADF_get_instance(ADF *adf, const uint32 instance_id) {
    ...
    return (ADFInstance *) DA_at(&adf->instances, instance_id);
}

void *ADF_read_instance(const ADF *adf, const ADFInstance *instance, const MemoryBuffer *mb,
                        const STITypeInfoMap *type_map) {
    TracyCZoneN(ctx, "ADF_read_instance", 1);
    const STITypeInfo **type_ptr = (const STITypeInfo **)DM_get(type_map, instance->type_hash);
    if (type_ptr == NULL) {
...
void ADF_print_instance(const ADFInstance *instance, const void *instance_data, JsonContext *ctx,
                        const STITypeInfoMap *type_map) {
    const STITypeInfo **type_ptr = (const STITypeInfo **)DM_get(type_map, instance->type_hash);
    if (type_ptr != NULL) {
