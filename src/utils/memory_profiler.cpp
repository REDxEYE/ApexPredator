...
void *tracy_xmalloc_dbg(size_t n, const char *file, const uint32_t line, const char *func) {
    ...
    if (p) {
        ...
        if (!mp_track_alloc(g_mp, p, n, {file, func, line}, &is)) mp_report(&is);
        TracyCAlloc(p, n);
    }
    return p;
}
...
void tracy_xfree_dbg(void *p, const char *file, const uint32_t line, const char *func) {
    ...
    if (!mp_track_free(g_mp, p, {file, func, line}, &is)) mp_report(&is);
    TracyCFree(p);
    free(p);
}
...
void *tracy_xrealloc_dbg(void *p, const size_t n, const char *file, const uint32_t line, const char *func) {
    ...
    if (p == NULL) {
        if (np) {
            MpTrackIssue is = {};
            if (!mp_track_alloc(g_mp, np, n, {file, func, line}, &is)) mp_report(&is);
            TracyCAlloc(np, n);
        }
        return np;
    }
    ...
    MpTrackIssue is = {};
    if (np==p) {
        if (!mp_track_resize(g_mp, p, n, {file, func, line}, &is)) mp_report(&is);
    } else {
        if (!mp_track_free(g_mp, p, {file, func, line}, &is)) mp_report(&is);
        if (!mp_track_alloc(g_mp, np, n, {file, func, line}, &is)) mp_report(&is);
    }
...
void *tracy_xcalloc_dbg(const size_t count, const size_t size, const char *file, const uint32_t line,
    ...
    if (!mp_track_alloc(g_mp, p, n, {file, func, line}, &is))
        mp_report(&is);
