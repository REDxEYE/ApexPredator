...
static void BeforeName(JsonContext *ctx) {
    JsonScope scope = ctx->scopes[ctx->index - 1];

    if (scope == JsonScope::JsonScope_NonEmptyObject) {
        fputs(ctx->compact ? ", " : ",", ctx->stream);
    }
    else if (scope != JsonScope::JsonScope_EmptyObject) {
        assert(0 && "Nesting problem");
    }

    NewLine(ctx);
    ReplaceTop(ctx, JsonScope::JsonScope_DanglingName);
}

static void BeforeValue(JsonContext *ctx) {
    switch (ctx->scopes[ctx->index - 1]) {
        case JsonScope::JsonScope_EmptyDocument:
            ReplaceTop(ctx, JsonScope::JsonScope_NonEmptyDocument);
            break;
        case JsonScope::JsonScope_NonEmptyDocument:
            assert(0 && "JSON must have only one top-level value");
            break;
        case JsonScope::JsonScope_EmptyArray:
            ReplaceTop(ctx, JsonScope::JsonScope_NonEmptyArray);
            NewLine(ctx);
            break;
        case JsonScope::JsonScope_NonEmptyArray:
            fputs(ctx->compact ? ", " : ",", ctx->stream);
            NewLine(ctx);
            break;
        case JsonScope::JsonScope_DanglingName:
            ReplaceTop(ctx, JsonScope::JsonScope_NonEmptyObject);
            fputs(": ", ctx->stream);
            break;
        default:
            assert(0 && "Nesting problem");
...
void jsonInit(JsonContext *ctx, FILE *stream) {
    ...
    Push(ctx, JsonScope::JsonScope_EmptyDocument);
}

void jsonBeginObject(JsonContext *ctx) {
    WriteDeferredName(ctx);
    Open(ctx, JsonScope::JsonScope_EmptyObject, '{');
}

void jsonEndObject(JsonContext *ctx) {
    Close(ctx, JsonScope::JsonScope_EmptyObject, JsonScope::JsonScope_NonEmptyObject, '}');
}

void jsonBeginArray(JsonContext *ctx) {
    WriteDeferredName(ctx);
    Open(ctx, JsonScope::JsonScope_EmptyArray, '[');
}

void jsonEndArray(JsonContext *ctx) {
    Close(ctx, JsonScope::JsonScope_EmptyArray, JsonScope::JsonScope_NonEmptyArray, ']');
}
