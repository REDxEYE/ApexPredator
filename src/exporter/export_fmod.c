...
int rebuild_fsb_vorbis_to_ogg(const Sample *s, MemoryBuffer *out) {
    ...
    ogg_packet id = {0}, comment = {0}, setup = {0};
    ...
    uint16_t pkt_size = rd_u16le(s->data + off);
    off += 2;
...
void process_fsb(RIFFContext *ctx, MemoryBuffer *mb) {
    ...
    for (int i = 0; i < header.sample_count; ++i) {
        ...
        Sample *sample = DA_append_get(&ctx->samples);
        ...
        while (metadata_header.has_data) {
            SampleMetadataChunk *chunk = DA_append_get(&metadata->chunks);
            ...
            chunk->data = mp_malloc(chunk->header.chunk_size);
    ...
    uint32 *name_offsets = mp_malloc(header.sample_count * sizeof(uint32));
    ...
    char *names = mp_malloc(header.name_size - header.sample_count * sizeof(uint32));
    ...
    for (int i = 0; i < header.sample_count; ++i) {
        ...
        sample->data = mp_malloc(sample_size);
