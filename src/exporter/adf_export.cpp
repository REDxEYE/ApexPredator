...
bool decompress_data(const CompressedData *data, MemoryBuffer *mb) {
    ...
    switch (compressed_header->comp_type) {
        case zstd: {
            ...
            const ZSTD_ErrorCode error = static_cast<ZSTD_ErrorCode>(ZSTD_isError(zstd_res));
...
Texture *export_terrain_texture(const TerrainTexture *terrain_texture, uint32 expected_channels,
    ...
    MemoryBuffer decompressed_buffer = {};
...
void export_terrain_patch(AppState *app_state, const StreamPatchBlockHeader *header,
    ...
    vec3 patch_pos = {(float32) patch_x_pos * 200.f, 0.0f, (float32) patch_z_pos * 200.f};
    glm_translate(patch_matrix, patch_pos);
...
void export_terrain_instances(const GLTFContext *context, ArchiveManager *archive_manager,
    ...
    for (int i = 0; i < instance_data_patch->InstanceDataLayers.count; ++i) {
        const InstanceDataLayer *instance_layer = static_cast<const InstanceDataLayer *>(DA_at(&instance_data_patch->InstanceDataLayers, i));
        const StringView layer_name = find_name32_sv(instance_layer->Name);
        // uint32 used_type = 0;
        for (int j = 0; j < instance_layer->Instances.count; ++j) {
            const VegetationSystemInstance *veg_instance = static_cast<const VegetationSystemInstance *>(DA_at(&instance_layer->Instances, j));
            // if (used_type==0) {
            ...
            glm_translate(instance_matrix, tranlation);
...
GL_ID export_stream_path_file(AppState *app_state, ADF *adf,
    ...
    const ADFInstance *path_file_header_instance = static_cast<const ADFInstance *>(DA_at(&adf->instances, 0));
    StreamPatchFileHeader *path_file_header = static_cast<StreamPatchFileHeader *>(ADF_read_instance(adf, path_file_header_instance, adf_buffer,
        &ADF_TYPES_type_info));
    ...
    for (int i = 1; i < adf->instances.count; ++i) {
        const ADFInstance *patch_instance = static_cast<const ADFInstance *>(DA_at(&adf->instances, i));
        ...
        if (patch_instance->type_hash == STI_TYPE_HASH_StreamPatchBlockHeader) {
            const StreamPatchBlockHeader *block_header = static_cast<const StreamPatchBlockHeader *>(instance_data);
            const ADFInstance *block_data_instance = static_cast<const ADFInstance *>(DA_at(&adf->instances, i+1));
...
GL_ID export_adf_file_from_buffer(AppState *app_state, const uint32 path_hash, MemoryBuffer *mb) {
    ...
    for (int instanceId = 0; instanceId < adf.header.instance_count; instanceId++) {
        const ADFInstance *instance = static_cast<const ADFInstance *>(DA_at(&adf.instances, instanceId));
        if (instance->type_hash == STI_TYPE_HASH_WorldSettings) {
            const WorldSettings *world_settings = static_cast<const WorldSettings *>(ADF_read_instance(&adf, instance, mb, &ADF_TYPES_type_info));
            ...
            const AmfModel *model = static_cast<const AmfModel *>(ADF_read_instance(&adf, instance, mb, &ADF_TYPES_type_info));
            ...
            instanceId++;
            const AmfMeshHeader *mesh_header = static_cast<const AmfMeshHeader *>(ADF_read_instance(&adf, instance, mb, &ADF_TYPES_type_info));
            const ADFInstance *mesh_buffers_instance = static_cast<const ADFInstance *>(DA_at(&adf.instances, instanceId));
            const AmfMeshBuffers *mesh_buffers = static_cast<const AmfMeshBuffers *>(ADF_read_instance(&adf, mesh_buffers_instance, mb,
                &ADF_TYPES_type_info));
